...
import SelectProblemModal from "../components/SelectProblemModal";
...
function SessionPage() {
  ...
  const updateProblemMutation = useUpdateSessionProblem();
  const autoJoinSessionIdRef = useRef(null);

  const session = sessionData?.session;
  const myGoogleId = user?.googleId;
  const isHost = session?.host?.googleId === myGoogleId;
  const isParticipant = session?.participant?.googleId === myGoogleId;
  ...
  const handleSelectProblem = ({ problem, difficulty }) => {
    updateProblemMutation.mutate({ id, problem, difficulty });
  };

  // host must pick a problem before the interview can start
  const needsProblemSelection = session && session.status === "active" && !session.problem;

  if (needsProblemSelection && isHost) {
    return (
  ...
  if (needsProblemSelection && isParticipant) {
    return (
      <div className="h-screen bg-base-100 flex flex-col">
        <Navbar />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-lg">Waiting for the interviewer to select a problem...</p>
          </div>
        </div>
      </div>
    );
